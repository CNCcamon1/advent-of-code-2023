...
int duplicate_card(int cardNumber, int* cardValues, int* numCards,
    int* cardsToDuplicate){
    //Add the card copies to the list
    int cardWorth = cardValues[cardNumber];
    for(int i=0; i<cardWorth; i++){
        numCards[cardNumber + i + 1] += 1;
        cardsToDuplicate[cardNumber + i + 1] += 1;
    }
...
int main(){
    ...
    int numCards [NUM_CARDS] = {0};
    int cardsToDuplicate [NUM_CARDS] = {0};
    int cardValues [NUM_CARDS] = {0};
    ...
    while((read = getline(&line, &len, fp)) != -1){
        ...
        //Save that value so it doesn't need to be recalculated later
        cardValues[currentCardIndex] = cardWorth;
        numCards[currentCardIndex] = 1;
        cardsToDuplicate[currentCardIndex] = 1;
        currentCardIndex++;
    }

    int remainingCardsToDuplicate = 1;
    while(remainingCardsToDuplicate > 0){
        remainingCardsToDuplicate = 0;
        for(int i=0; i<NUM_CARDS; i++){
            for(int j=0; j<cardsToDuplicate[i]; j++){
                int cardsDuplicated = duplicate_card(
                    i, &cardValues, &numCards, &cardsToDuplicate);
                if(cardsDuplicated > 0){
                    remainingCardsToDuplicate = 1;
                }
            }

            cardsToDuplicate[i] = 0;
        }
    }

    int totalWorth = 0;
    for(int i=0; i<NUM_CARDS; i++){
        totalWorth += numCards[i];
    }
